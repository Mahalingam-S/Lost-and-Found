...
import { PlusCircle, MapPin, Calendar, ArrowLeft, Check, UploadCloud, X } from 'lucide-react';
...
export default function ReportFound({ onSuccess, onCancel }) {
  const { user } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setError(res.message || 'Failed to post found item');
      }
    } catch (err) {
      ...
      <div className="glass-card" style={{ padding: '20px', background: '#ffffff', border: '1px solid #e2e8f0' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '18px' }}>
          ...
          <div>
            ...
            <p style={{ color: '#64748b', fontSize: '0.78rem', fontWeight: 500 }}>
              Found something? Post it to reunite with owner
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <div>
            ...
            <input
              type="text"
              placeholder="e.g. Student ID / Yamaha Keys / Casio Calculator"
              value={title}
              ...
              {DEMO_IMAGES.map((imgUrl, i) => (
                <div
                  ...
                  style={{
                    ...
                    overflow: 'hidden',
                    cursor: 'pointer',
                    border: image === imgUrl ? '2px solid #059669' : '1px solid #cbd5e1',
                    position: 'relative',
                    flexShrink: 0
                  }}
                >
          ...
          <div>
            ...
            <textarea
              placeholder="Describe where it is stored or security desk handover..."
              value={description}
